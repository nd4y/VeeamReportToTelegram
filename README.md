...
`JobName` должен точно совпадать с именем задания в Veeam. Параметр не обязателен — если он не задан, для всех заданий используются пороги по умолчанию.
...
2. Токен Telegram-бота и ID чата, а также кастомные пороги RPO читаются из `VeeamReport.conf` (см. [Настройку](#настройка-veeamreportconf)).
